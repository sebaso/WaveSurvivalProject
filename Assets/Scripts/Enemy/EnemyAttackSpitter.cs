...
public class EnemyAttackSpitter : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (enemy.player != null && Vector3.Distance(transform.position, enemy.player.position) <= attackRange)
            {
    ...
    public void Shoot(GameObject target)
    {
        if (Time.time > nextAttack)
        {
            ...
            projectile.GetComponent<EnemyProjectile>().target = target.transform;
            StartCoroutine(AttackInPlace());
        }
    }
